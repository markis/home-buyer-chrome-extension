...
define(['backbone', 'modules/SpreadsheetProvider'], function(Backbone, SpreadsheetProvider) {
	var CreateSpreadsheetButtonView = Backbone.View.extend({
		tagName: 'button',
		...
		render: function() {
			this.$el.text('Create first spreadsheet');
			return this;
		},
		setupAuth:  function() {
			var self = this;
			self.$el.attr('diabled', true);
		    self.$el.text('Creating first spreadsheet...');
		    var spreadsheetProvider = new SpreadsheetProvider();
		    spreadsheetProvider.createHomeBuyerSpreadsheet('Home Buyer', function() {
	...
	return CreateSpreadsheetButtonView;
});